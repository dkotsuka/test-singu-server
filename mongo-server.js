...
router.put('/put', function(req, res) {
	...
	client.connect(err => {
		const collection = client.db(DATABASE).collection(VOUCHERS);
		updateDocument(collection, voucher, function(res) {
			return res.json(res)  // VERIFICAR SE É ISSO MESMO.
			client.close()
		})
	})
})
...
router.delete('/delete', function(req, res) {
	...
	client.connect(err => {
		const collection = client.db(DATABASE).collection(VOUCHERS);
		removeDocument(collection, voucher._id, function(res) {
			return res.json(res)  // VERIFICAR SE É ISSO MESMO.
			client.close()
		})
	})
...
router.put('/disable', function(req, res) {
	...
	client.connect(err => {
		const collection = client.db(DATABASE).collection(VOUCHERS);
		disableDocument(collection, data, function(response) {
...
const updateDocument = function(collection, voucher, callback) {
	collection.updateOne({'_id': ObjectID(voucher._id)}, { $set: voucher }, function (err, success) {
		assert.equal(err, null)
		assert.equal(1, success.result.n)
